...
export async function checkAuth() {
    ...
    // "commit" резолвится как только пришёл ответ навигации (после серверных
    // редиректов) — не ждём полной загрузки DOM/JS. Для проверки авторизации
    // достаточно итогового URL (редирект на /account/login = не залогинен).
    // Это устойчивее к медленной сети/VPN, чем domcontentloaded c таймаутом 30с.
    try {
        ...
        // навигация подвисла — попробуем прочитать текущий URL всё равно
    }
    // дать серверному редиректу/DOM осесть (не критично — игнорируем таймаут)
...
export async function searchVacancies(params) {
    ...
    // experience/schedule/work_format могут быть строкой ИЛИ массивом значений —
    // hh.ru принимает повторяющиеся параметры (experience=...&experience=...)
    const appendMulti = (key, val) => {
        if (val === undefined || val === null)
            return;
        const arr = Array.isArray(val) ? val : [val];
        arr.forEach((x) => searchUrl.searchParams.append(key, x));
    };
    appendMulti("experience", params.experience);
    if (params.employment)
        searchUrl.searchParams.set("employment", params.employment);
    appendMulti("schedule", params.schedule);
    appendMulti("work_format", params.work_format);
    if (params.search_field) {
        // matching по конкретным полям (name = только название вакансии → релевантность)
        const fields = Array.isArray(params.search_field) ? params.search_field : [params.search_field];
        fields.forEach((f) => searchUrl.searchParams.append("search_field", f));
    }
    if (params.order_by)
        searchUrl.searchParams.set("order_by", params.order_by);
    if (params.page)
    ...
    // Ждём, пока выдача дорисуется. Фиксированной паузы мало: под нагрузкой hh
    // успевает отрисовать 20 карточек из 50, и мы молча теряли остальные —
    // страница выглядела «последней», хотя за ней были ещё вакансии.
    // Условие выхода: набрали сколько просили ИЛИ счётчик перестал расти.
    {
        const want = params.perPage || 50;
        const countCards = () => page.evaluate(() => document.querySelectorAll('[data-qa="vacancy-serp__vacancy"]').length);
        let prev = -1;
        let stable = 0;
        for (let i = 0; i < 15; i++) {
            await page.waitForTimeout(600);
            const n = await countCards().catch(() => 0);
            if (n >= want)
                break;
            stable = n === prev ? stable + 1 : 0;
            if (n > 0 && stable >= 3)
                break; // счётчик замер — это правда конец выдачи
            prev = n;
        }
    }
...
export async function getVacanciesDetails(vacancyIds) {
    // Пачкой в одном процессе: браузер поднимается один раз на весь список.
    // Поштучный вызов через драйвер стоил ~35с на вакансию (запуск Chrome +
    // загрузка сессии на каждую), пачкой — секунды.
    const results = [];
    for (const id of vacancyIds || []) {
        try {
            results.push(await getVacancyDetails(id));
        }
        catch (e) {
            results.push({ id: String(id), error: String(e?.message || e) });
        }
    }
    return results;
}
export async function getResumes() {
    ...
    // Карточки резюме на hh догружаются JS-ом после domcontentloaded —
    // ждём появления ссылок на резюме, иначе вернётся пустой список.
    ...
    const resumes = await page.evaluate(() => {
        ...
        if (!items || items.length === 0) {
            ...
            links.forEach((link) => {
                ...
                // Служебные ссылки вида /resume/edit/<id>/visibility («Сделать видимым»)
                // раньше попадали в список как резюме с id "ed" — из слова "edit".
                if (/\/resume\/(edit|converter|search)/i.test(href))
                    return;
                const idMatch = href.match(/\/resume\/([a-f0-9]{20,})/i);
                if (!idMatch || results.find((r) => r.id === idMatch[1]))
                    return;
                // Карточка внутри ссылки: [тип занятости, НАЗВАНИЕ, зарплата · формат].
                // Раньше брали textContent целиком и получали склейку без пробелов.
                const lines = (link.innerText || "").split("\n")
                    .map((x) => x.trim()).filter(Boolean);
                results.push({
                    id: idMatch[1],
                    title: lines[1] || lines[0] || "Резюме",
                    employment: lines[0] || "",
...
export async function getApplications() {
    ...
    // Отклики разложены по страницам (20 на страницу, вкладка «Все» активна по
    // умолчанию). Раньше читалась только первая — из 122 откликов мониторинг
    // видел 20, и приглашения по остальным были невидимы.
    const scrape = () => page.evaluate(() => {
        const items = document.querySelectorAll('[data-qa="negotiations-item"], [data-qa="negotiations-list-item"]');
        const results = [];
        items.forEach((item) => {
            const titleEl = item.querySelector('[data-qa="negotiations-item-vacancy"], [data-qa="negotiations-list-item-vacancy"]');
            const companyEl = item.querySelector('[data-qa="negotiations-item-company"], [data-qa="negotiations-list-item-employer"]');
            const dateEl = item.querySelector('[data-qa="negotiations-item-date"], [data-qa="negotiations-list-item-date"]');
            const tagEl = item.querySelector('[data-qa^="negotiations-tag"], [data-qa="negotiations-list-item-status"]');
            const link = titleEl?.href || titleEl?.closest("a")?.href
                || item.querySelector('a[href*="/vacancy/"]')?.href || "";
            const title = titleEl?.textContent?.trim() || "";
            if (title)
                results.push({
                    title,
                    company: companyEl?.textContent?.trim() || "",
                    status: (tagEl?.textContent || "").replace(/ /g, " ").trim(),
    ...
    const collected = [];
    const seen = new Set();
    const maxPages = 25; // 20 на страницу — с запасом на 500 откликов
    for (let p = 0; p < maxPages; p++) {
        if (p > 0) {
            const url = `${BASE_URL}/applicant/negotiations?page=${p}`;
            await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
            await page.waitForTimeout(1800);
        }
        const batch = await scrape().catch(() => []);
        let added = 0;
        for (const item of batch) {
            const key = `${item.company}|${item.title}|${item.date}`;
            if (!seen.has(key)) {
                seen.add(key);
                collected.push(item);
                added++;
        ...
        // страница без новых записей = выдача кончилась (или hh повторил последнюю)
        if (added === 0)
            break;
    }
    return collected;
